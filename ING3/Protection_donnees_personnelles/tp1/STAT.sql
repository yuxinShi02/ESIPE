...
begin
  res := 0 ;
  execute immediate 'select count(*) from STATS_VIEW where ' || where_clause || ' ' into res ;
  return res ;
end;
/
...
begin
  res := 0 ;
  execute immediate 'select sum(leucocyte)  from STATS_VIEW where ' || where_clause || ' ' into res ;
  return res ;
end;
/
